...
def des_rho(total_neon, degas):
    ...
    leng = 10**5
    des_rho = [0] * len(M_co)
    rho_den = np.logspace(-18, -5, leng)

    #print(rho,M_co,rho_den)
    ...
    for j in range(len(M_co)):
        #index = 0
        for i in range(leng):
            #print(f'is: {M_co[j], index, i}')
            if np.abs((f(M_co[j], rho_den[leng - 1 - i - index]) - f3(M_co[j]))) / f3(M_co[j]) < 0.01:
                #print(f3(M_co[j]), f(M_co[j], rho_den[i + index]))
                #print(M_co[j], rho_den[leng - 1 - i - index])
                des_rho[j] = rho_den[leng - 1- i - index ]
                index = i + index
                #print(i)
                break
    #print(des_rho/MMSN_V(0.1))
    return des_rho
